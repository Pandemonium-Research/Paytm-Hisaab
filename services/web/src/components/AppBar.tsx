...
export function AppBar({ title, backLabel, onBack, language, languageLabel, onLanguageClick, helpLabel, onHelp, trailing, className }: AppBarProps) {
  return (
    <header className={cn('safe-top flex min-h-[64px] items-center gap-2 border-b border-hairline bg-card px-2', className)}>
      ...
      <h1 className="min-w-0 flex-1 truncate text-base font-semibold text-navy">{title}</h1>
      {language && (
